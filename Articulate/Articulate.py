...
def articulate():
    ...
    global DK
    DK = []
    for DK_key in list(art.var_dict.keys()):
    ...
    searchwords = [] #list of search words to find in articles
    #This establishes the words being searched for in an article, search for direct quote
    for term_key in list(art.var_wordstore.keys()):
        if art.var_wordstore[term_key] != 0: #ignore unchecked boxes in GUI
    ...
    for search in searchwords:
        ...
        incl_list = []
        for incl_keys in list(art.var_inclstore.keys()):
    ...
    dates_range = pd.date_range(
        ...
        freq='M')#must enter a day from next month or last day of current month or else concatenates.
    ...
    for k in range(len(searchwords)): #for each searchword there is a new dictionary
        ...
        article_tally[search] = pd.DataFrame(np.zeros((len(dates_range),len(list(media_sites.keys())))),index=dates_range, columns=list(media_sites.keys())) 
        to_write[search] = {}
        for n in range(len(list(media_sites.keys()))): #for each site, creating a new dictionary
            row_num = 1
            site = media_sites[list(media_sites.keys())[n]]
                    ...
                    while True:
                        ...
                        while rerun_val == 1:
                            ...
                            res,rerun_val,DKcount,DKcheck,DKnum,current_day = art.runquery(
                                DK,
                                media_sites[list(media_sites.keys())[n]],
                                searchwords[k],
                                index,
                                date_start,
                                date_end,
                        ...
                        if n_res1 > 0 and n_res2 > 0:
                            for num in range(n_res1):
                                row,article_excluded,title_missed,article_tally[search],error,title_store[list(media_sites.keys())[n]] = art.getinfo(
                                    res,
                                    day_list,
                                    site,
                                    search,
                                    list(media_sites.keys()),
                                    n,
                                    num,
                                    commands,
                                    article_excluded,
                                    ...
                                    error,
                                    title_store[list(media_sites.keys())[n]],
                                    orterm_specific)
                            ...
                            break
                        if 'nextPage' not in list(res['queries'].keys()): #if there is no next page, end search
                            break
        ...
        article_tally[search]['Sum'] = (
            article_tally[search][list(article_tally[search].keys())[0:]].sum(1))
    ...
    with open(output_filename, 'wb') as fto:
        ...
        for srkey in list(to_write.keys()):
            for sikey in list(to_write[srkey].keys()):
                for inkey in list(to_write[srkey][sikey].keys()):
