...
def DKtest(DKcount,DKcheck,DKnum,current_day,DK,rerun_val):
    if DKcheck == DKcount or DKcount >= 10000:
        ...
        if DKnum >= len(DK): #if trying to go beyond number you actually have
            ...
            DKnum_holder = len(DK)
            while DKrun == 1:
                DKnum_holder = raw_input('Set DK (Developer Key) back to 0 or type "3" to exit:')
                print "You type:" + DKnum_holder
                if DKnum_holder < len(DK) and DKnum_holder >= 0:
                    ...
                    while datetime.now().day == current_day: #while today is still today
                        print 'You need wait like 24 hours to use your Developer Keys again. This is because you have reached a maximum of 100 API requests.'
                        time.sleep(300) #sleep for 300 seconds and then try again
                elif DKnum_holder =="3": #if you dont want to wait for developer keys to reset, type "Exit"
                    DKrun = 0
                    print 'exiting'
                    sys.exit()
                else:
